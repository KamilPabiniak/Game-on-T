...
public class ConnectionManager : MonoBehaviour
{
    ...
    private IEnumerator InitializeConnection()
    {
        ...
        {
            Debug.Log("Waiting for world initialization...");
            yield return null;
        }
        ...
        if (isServer)
        {
            ...
            var result = query.GetSingletonRW<NetworkStreamDriver>().ValueRW.Listen(NetworkEndpoint.Parse(_listenIP, _port));
            if (!result)
            {
                Debug.LogError($"Failed to start server on {_listenIP}:{_port}. Error code: {result}");
            }
            else
            {
                Debug.Log($"Server listening on {_listenIP}:{_port}");
            }
        }
        ...
        if (isClient)
        {
            ...
            var result = query.GetSingletonRW<NetworkStreamDriver>().ValueRW.Connect(ClientWorld.EntityManager, NetworkEndpoint.Parse(_connectIP, _port));
            if (result == Entity.Null)
            {
                Debug.LogError($"Failed to connect to server at {_connectIP}:{_port}. Connection Entity is null.");
            }
            else
            {
                Debug.Log($"Client connecting to {_connectIP}:{_port}");
            }
